...
void LBM::setup_subdomain()
{
    ...
    if (x_lo != 0)
    {
        m_left = buffer_t("m_left", q, l_l[1], l_l[2]);
        m_leftout = buffer_t("m_leftout", q, l_l[1], l_l[2]);
    }
    if (x_hi != glx - 1)
    {
        m_right = buffer_t("m_right", q, l_l[1], l_l[2]);
        m_rightout = buffer_t("m_rightout", q, l_l[1], l_l[2]);
    }
    if (z_lo != 0)
    {
        m_down = buffer_t("m_down", q, l_l[0], l_l[1]);
        m_downout = buffer_t("m_downout", q, l_l[0], l_l[1]);
    }
    if (z_hi != glz - 1)
    {
        m_up = buffer_t("m_up", q, l_l[0], l_l[1]);
        m_upout = buffer_t("m_upout", q, l_l[0], l_l[1]);
    }

    if (y_lo != 0)
    {
        m_front = buffer_t("m_front", q, l_l[0], l_l[2]);
        m_frontout = buffer_t("m_frontout", q, l_l[0], l_l[2]);
    }
    if (y_hi != gly - 1)
    {
        m_back = buffer_t("m_back", q, l_l[0], l_l[2]);
        m_backout = buffer_t("m_backout", q, l_l[0], l_l[2]);
    }
    // 12 lines
...
void LBM::pack()
{
    // 6 faces
    if (x_lo != 0)
        Kokkos::deep_copy(m_leftout, Kokkos::subview(f, Kokkos::ALL, l_s[0], std::make_pair(l_s[1], l_e[1]), std::make_pair(l_s[2], l_e[2])));

    if (x_hi != glx - 1)
        Kokkos::deep_copy(m_rightout, Kokkos::subview(f, Kokkos::ALL, l_e[0] - 1, std::make_pair(l_s[1], l_e[1]), std::make_pair(l_s[2], l_e[2])));

    if (z_lo != 0)
        Kokkos::deep_copy(m_downout, Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), std::make_pair(l_s[1], l_e[1]), l_s[2]));

    if (z_hi != glz - 1)
        Kokkos::deep_copy(m_upout, Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), std::make_pair(l_s[1], l_e[1]), l_e[2] - 1));

    if (y_lo != 0)
        Kokkos::deep_copy(m_frontout, Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), l_s[1], std::make_pair(l_s[2], l_e[2])));

    if (y_hi != gly - 1)
        Kokkos::deep_copy(m_backout, Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), l_e[1] - 1, std::make_pair(l_s[2], l_e[2])));
...
void LBM::unpack()
{
    // 6 faces
    if (x_lo != 0)
        Kokkos::deep_copy(Kokkos::subview(f, Kokkos::ALL, l_s[0], std::make_pair(l_s[1], l_e[1]), std::make_pair(l_s[2], l_e[2])), m_left);

    if (x_hi != glx - 1)
        Kokkos::deep_copy(Kokkos::subview(f, Kokkos::ALL, l_e[0], std::make_pair(l_s[1], l_e[1]), std::make_pair(l_s[2], l_e[2])), m_right);

    if (z_lo != 0)
        Kokkos::deep_copy(Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), std::make_pair(l_s[1], l_e[1]), l_s[2] - 1), m_down);

    if (z_hi != glz - 1)
        Kokkos::deep_copy(Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), std::make_pair(l_s[1], l_e[1]), l_e[2]), m_up);

    if (y_lo != 0)
        Kokkos::deep_copy(Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), l_s[1] - 1, std::make_pair(l_s[2], l_e[2])), m_front);

    if (y_hi != gly - 1)
        Kokkos::deep_copy(Kokkos::subview(f, Kokkos::ALL, std::make_pair(l_s[0], l_e[0]), l_e[1], std::make_pair(l_s[2], l_e[2])), m_back);
    // 12 lines
    if (x_lo != 0 && z_hi != glz - 1)
